		...
		if(toggleTileCacheVar) {
			...
			for(x = offx-(offx%4); x <= offx+offw; x+=1) {
				for(y = offy; y <= offy+offh; y+=1) {
					if(x >= lw || y >= lh) continue;
					if(x % 4 === 0) {
						lc.strokeStyle = 'rgb(255, 0, 0)';
						lc.lineWidth = 1*zoomlevel;
						lc.strokeRect(Math.floor(x*32*zoomlevel-scrollX)-0.5*zoomlevel, Math.floor((y)*32*zoomlevel-scrollY)-0.5*zoomlevel, 4*32*zoomlevel, 32*zoomlevel);
						lc.strokeStyle = 'rgba(100, 50, 0, 0.75)';
						lc.lineWidth = 4*zoomlevel;
						lc.fillStyle = "#FB0";
						lc.font = "bold "+Math.round(20*zoomlevel)+"px sans-serif";
						if(dynamicTileCache[l] && dynamicTileCache[l][x/4+realWidth*y] !== undefined) {
							lc.strokeText(dynamicTileCache[l][x/4+realWidth*y], Math.floor((x*32+2.5)*zoomlevel-scrollX), Math.floor((y*32+17.5)*zoomlevel-scrollY));
							lc.fillText(dynamicTileCache[l][x/4+realWidth*y], Math.floor((x*32+2)*zoomlevel-scrollX), Math.floor((y*32+17)*zoomlevel-scrollY));
						}
					}
					if(J2L.LEVEL[l][x][y].animated) {
						lc.strokeStyle = 'rgba(100, 0, 100, 0.75)';	
						lc.lineWidth = 4*zoomlevel;
						lc.fillStyle = "#F0F";
						lc.font = "bold "+Math.round(16*zoomlevel)+"px sans-serif";
						lc.strokeText("a", Math.floor((x*32+20.5)*zoomlevel-scrollX), Math.floor((y*32+20+9.5)*zoomlevel-scrollY));
						lc.fillText("a", Math.floor((x*32+20)*zoomlevel-scrollX), Math.floor((y*32+20+9)*zoomlevel-scrollY));
					}
				}
			}
			lc.restore();
